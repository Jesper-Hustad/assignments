...
export default {
    ...
    methods: {  
        ...
        addResult(result, color){
            document.querySelector('.result-history').innerHTML += `<span style="color : ${color}">${result}</span><br>`
        ...
        async queryServer(equation){
            ...
            const response = await fetch("/api/v1/equation?" + new URLSearchParams(params).toString())
